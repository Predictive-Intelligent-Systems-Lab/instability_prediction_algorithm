...
class CaoMethodProcessor:
    def __init__(self, input_folder=None, output_folder=None, 
        ...
        """
        Initialize the Cao method processor with paths relative to the script location.
        
        Args:
            input_folder: Path to folder containing pressure segments
            output_folder: Path to save segments with embedding dimensions
            max_dim: Maximum embedding dimension to consider
            threshold: Convergence threshold for E1 values
        """
        ...
        if input_folder is None:
            self.input_folder = os.path.join(script_dir, "pressure_segments")
        else:
            self.input_folder = input_folder
            
        if output_folder is None:
            # Create data_with_embed folder at the same level as the script, not inside pressure_segments
            self.output_folder = os.path.join(script_dir, "data_with_embed")
        else:
    ...
    def process_segments(self):
        ...
        segment_files = [f for f in os.listdir(self.input_folder) if f.endswith(".npy")]
        ...
        for file_name in sorted(segment_files):
            try:
    ...
    def print_summary(self):
        ...
        print("Format: U0_[value]_segment_[index]_embed_[dim].npy")
