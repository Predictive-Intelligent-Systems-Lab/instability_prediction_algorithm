...
class SegmentedAMIAnalyzer:
    ...
    def __init__(self, input_folder=None, output_folder=None, 
        ...
        if input_folder is None:
            self.input_folder = os.path.join(script_dir, "data_with_embed")
        else:
            self.input_folder = input_folder
            
        if output_folder is None:
            self.output_folder = os.path.join(script_dir, "data_with_embed_tau")
        else:
    ...
    def process_segmented_signals(self):
        ...
        for file_name in sorted(segment_files):
            try:
                ...
                # Format should be: U0_[value]_segment_[index]_embed_[dim].npy
                parts = os.path.splitext(file_name)[0].split('_')
                
                # Extract relevant components - adjusted for new file naming structure
                base_name = os.path.splitext(file_name)[0]  # Without extension
                embed_dim = parts[-1]  # Last part should be the embedding dimension
                
                # Create new filename with both embedding dimension and tau
                new_filename = f"{base_name}_tau_{optimal_tau}.npy"
    ...
    def print_summary(self):
        ...
        print("Format: U0_[value]_segment_[index]_embed_[dim]_tau_[value].npy")
